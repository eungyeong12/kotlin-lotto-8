...
value class BonusNumber private constructor(
    ...
    companion object {
        fun from(input: String, winningNumber: WinningNumber): BonusNumber {
            validateNotBlank(input, ErrorMessage.BLANK_BONUS_NUMBER.errorMessage)
            val number = parseToNumber(input, ErrorMessage.BONUS_NUMBER_NOT_INTEGER.errorMessage)
        ...
        private fun validateBonusNumber(number: Int, winningNumber: WinningNumber) {
            require(number in LOTTO_MIN_NUMBER..LOTTO_MAX_NUMBER) {
                ErrorMessage.WRONG_RANGE_BONUS_NUMBER.errorMessage
            }
            require(!winningNumber.contains(number)) { ErrorMessage.DUPLICATE_BONUS_NUMBER.errorMessage }
        }
    }
}